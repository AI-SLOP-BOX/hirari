...
impl AutomationRecorderOrchestrator {
    ...
    pub fn record_value(&mut self, _track_id: u32, _param_id: u32, value: f32, timestamp: u64) {
        ...
        let tick_is_monotonic = self
            .lane_last_pos
            .get(&(_track_id, _param_id))
    ...
    pub fn preview_value(&mut self, value: f32, timestamp: u64) -> bool {
        if !value.is_finite() {
            return false;
        }
        self.preview = Some(RecordEvent {
            track_id: 0,
            param_id: 0,
            pos: timestamp,
            val: value,
    ...
    pub fn capture_preview(&mut self, track: u32, param: u32) -> bool {
        let Some(event) = self.preview.take() else {
            return false;
        };
        let before = self.events.len();
        self.record_value(track, param, event.val, event.pos);
        self.events.len() > before
    }
    pub fn set_max_events(&mut self, max_events: usize) -> bool {
        if max_events == 0 || max_events > 10_000_000 {
            return false;
        }
    ...
    pub fn clear_lane(&mut self, track_id: u32, param_id: u32) -> bool {
        let before = self.events.len();
        self.events
            .retain(|e| e.track_id != track_id || e.param_id != param_id);
        self.lane_last_pos.remove(&(track_id, param_id));
    ...
    pub fn transform_lane(
        ...
        if track_id == 0 || param_id == 0 || !scale.is_finite() || !offset.is_finite() {
            return false;
        }
        ...
        for event in self
            .events
            .iter_mut()
            .filter(|e| e.track_id == track_id && e.param_id == param_id)
        {
    ...
    pub fn clear_before(&mut self, timestamp: u64) -> usize {
        ...
        for event in &self.events {
            self.lane_last_pos
                .insert((event.track_id, event.param_id), event.pos);
        }
        before - self.events.len()
    }
    ...
    pub fn trim_lane(&mut self, track_id: u32, param_id: u32, start: u64, end: u64) -> bool {
        if track_id == 0 || param_id == 0 || end <= start {
            return false;
        }
        if !self
            .events
            .iter()
            .any(|event| event.track_id == track_id && event.param_id == param_id)
        {
            return false;
        }
        ...
        updated.retain(|event| {
            event.track_id != track_id
                ...
                || (event.pos >= start && event.pos <= end)
        });
        for event in updated
            .iter_mut()
            .filter(|event| event.track_id == track_id && event.param_id == param_id)
        {
    ...
    pub fn reverse_lane(&mut self, track_id: u32, param_id: u32, start: u64, end: u64) -> bool {
        ...
        let mut changed = false;
        for event in self.events.iter_mut().filter(|event| {
            event.track_id == track_id
                && event.param_id == param_id
                && event.pos >= start
                && event.pos <= end
        ...
        if changed {
            self.events
                .sort_by_key(|event| (event.track_id, event.param_id, event.pos));
            self.rebuild_lane_positions();
        }
        changed
    }
    ...
    pub fn flush(&mut self) {
        ...
        for curr in &self.events {
            if !curr.val.is_finite()
                || last_tick
                    .get(&(curr.track_id, curr.param_id))
                    .is_some_and(|tick| curr.pos < *tick)
            {
                continue;
            }

            let should_keep = thinned_events
                .iter()
                .rev()
                .find(|prev: &&RecordEvent| {
                    prev.track_id == curr.track_id && prev.param_id == curr.param_id
                })
                .is_none_or(|prev| (curr.val - prev.val).abs() > 0.001);

            // Thinning Logic: Skip redundant points where value hasn't changed significantly
            if should_keep {
                thinned_events.push(RecordEvent {
                    track_id: curr.track_id,
                    param_id: curr.param_id,
                    pos: curr.pos,
                    val: curr.val,
                });
            }
    ...
    pub fn sample_accurate_snapshot(&self) -> Vec<(u64, f32)> {
        self.events.iter().map(|e| (e.pos, e.val)).collect()
    }
    pub fn lane_snapshot(&self, track_id: u32, param_id: u32) -> Vec<(u64, f32)> {
        self.events
            .iter()
            .filter(|e| e.track_id == track_id && e.param_id == param_id)
            .map(|e| (e.pos, e.val))
    ...
    pub fn audit_automation_recorder(&self) -> bool {
        ...
        self.max_events > 0
            ...
            && self
                .events
                .iter()
                .all(|event| event.track_id != 0 && event.param_id != 0)
            && self.events.iter().all(|event| event.val.is_finite())
            && self.events.iter().enumerate().all(|(i, event)| {
                self.events[..i]
                    .iter()
                    .filter(|previous| {
                        previous.track_id == event.track_id && previous.param_id == event.param_id
                    })
                    .all(|previous| previous.pos <= event.pos)
            })
    }
}
