...
impl SnapshotOrchestrator {
    ...
    pub fn take_snapshot(&mut self, name: &str, states: HashMap<u32, f32>) {
        if name.trim().is_empty() || name.len() > 128 || states.len() > 65_536
            || states.values().any(|value| !value.is_finite()) {
            return;
        }
        if let Some(existing) = self.snapshots.iter_mut().find(|snapshot| snapshot.name.eq_ignore_ascii_case(name.trim())) {
    ...
    pub fn take_snapshot_with_plugins(
        ...
        if plugin_states.len() > 65_536 || plugin_states.iter().any(|state| !validate_plugin_state(state)) {
            return;
        }
        self.take_snapshot(name, states);
        if let Some(snapshot) = self.snapshots.iter_mut().find(|snapshot| snapshot.name.eq_ignore_ascii_case(name.trim())) {
    ...
    pub fn diff_snapshots(&self, idx1: usize, idx2: usize) -> Vec<(u32, f32, f32)> {
        ...
        let mut ids = s1.parameter_states.keys().chain(s2.parameter_states.keys()).copied().collect::<Vec<_>>();
        ids.sort_unstable();
        ids.dedup();
        for id in ids {
            let val1 = s1.parameter_states.get(&id).copied().unwrap_or(0.0);
            let val2 = s2.parameter_states.get(&id).copied().unwrap_or(0.0);
            if (val1 - val2).abs() > 1e-6 { diff.push((id, val1, val2)); }
        }
    ...
    pub fn list_names(&self) -> Vec<String> {
        let mut names: Vec<_> = self.snapshots.iter().map(|snapshot| snapshot.name.clone()).collect();
    ...
    pub fn rename_snapshot(&mut self, old: &str, new: &str) -> bool {
        let new = new.trim();
        if new.is_empty() || new.len() > 128 || new.contains('\0') { return false; }
        let Some(index) = self.snapshots.iter().position(|s| s.name.eq_ignore_ascii_case(old.trim())) else { return false; };
        if self.snapshots[index].name.eq_ignore_ascii_case(new) { return true; }
        if self.snapshots.iter().any(|s| s.name.eq_ignore_ascii_case(new)) { return false; }
    ...
    pub fn remove_snapshot(&mut self, name: &str) -> bool {
        ...
        self.snapshots.retain(|snapshot| !snapshot.name.eq_ignore_ascii_case(name.trim()));
    ...
    pub fn restore_snapshot(&self, name: &str) -> Option<(HashMap<u32, f32>, Vec<PluginSnapshotState>, String)> {
        let snapshot = self.snapshots.iter().find(|snapshot| snapshot.name.eq_ignore_ascii_case(name.trim()))?;
        Some((snapshot.parameter_states.clone(), snapshot.plugin_states.clone(), snapshot.routing_state.clone()))
    }
    ...
    pub fn diff_plugins(&self, idx1: usize, idx2: usize) -> Vec<PluginSnapshotState> {
        let (Some(left), Some(right)) = (self.snapshots.get(idx1), self.snapshots.get(idx2)) else { return Vec::new(); };
        let mut keys: Vec<_> = left.plugin_states.iter().map(|s| (s.track_id, s.plugin_index))
            .chain(right.plugin_states.iter().map(|s| (s.track_id, s.plugin_index))).collect();
        keys.sort_unstable(); keys.dedup();
        keys.into_iter().filter_map(|(track_id, plugin_index)| {
            let a = left.plugin_states.iter().find(|s| s.track_id == track_id && s.plugin_index == plugin_index);
            let b = right.plugin_states.iter().find(|s| s.track_id == track_id && s.plugin_index == plugin_index);
            if a == b { None } else { b.or(a).cloned() }
        }).collect()
    }
        ...
        self.snapshots.len() <= 65_536
            && self.snapshots.iter().enumerate().all(|(i, snapshot)| {
                !snapshot.name.trim().is_empty()
                    ...
                    && self.snapshots[..i].iter().all(|previous| !previous.name.eq_ignore_ascii_case(&snapshot.name))
            })
    }
}

fn validate_plugin_state(state: &PluginSnapshotState) -> bool {
    state.track_id != 0 && state.parameters.len() <= 65_536 && state.parameters.iter().all(|v| v.is_finite())
}
