...
mod tests {
    ...
    fn built_in_genre_templates_are_valid_and_resolvable() {
        ...
        assert_eq!(
            orchestrator
                .try_instantiate_template("Electronic")
                .unwrap()
                .initial_bpm,
            124
        );
...
impl TemplateOrchestrator {
    ...
    pub fn starter_templates() -> Vec<ProjectTemplate> {
        let bus = |name: &str| TrackDef {
            name: name.into(),
            r#type: TrackType::Bus,
            insert_effects: vec![],
        };
        let instrument = |name: &str| TrackDef {
            name: name.into(),
            r#type: TrackType::Instrument,
            insert_effects: vec![],
        };
        vec![
            ProjectTemplate {
            ...
            ProjectTemplate {
                name: "Singer-Songwriter".into(),
                initial_bpm: 92,
                tracks: vec![
                    TrackDef {
                        name: "Vocal".into(),
                        r#type: TrackType::Audio,
                        insert_effects: vec!["Aura/DeEsser".into()],
                    },
                    instrument("Piano"),
                    bus("Mix Bus"),
                ],
            ...
            ProjectTemplate {
                ...
                tracks: vec![
                    TrackDef {
                        name: "Voice".into(),
                        r#type: TrackType::Audio,
                        insert_effects: vec!["Aura/Compressor".into(), "Aura/DeEsser".into()],
                    },
                    bus("Print Master"),
                ],
            },
    ...
    pub fn load_templates(&mut self, json_data: &str) -> anyhow::Result<()> {
        ...
        if loaded.is_empty() || loaded.len() > 256 {
            anyhow::bail!("template collection size is invalid");
        }
        if loaded.iter().enumerate().any(|(i, t)| {
            loaded[..i]
                .iter()
                .any(|p| p.name.eq_ignore_ascii_case(t.name.trim()))
        }) {
            anyhow::bail!("duplicate template name");
        }
        if !loaded.iter().all(|t| {
            !t.name.trim().is_empty()
                && t.name.len() <= 256
                && t.initial_bpm > 0
                && t.initial_bpm <= 1_000
                && t.tracks.len() <= 512
                && t.tracks.iter().all(|track| {
                    !track.name.trim().is_empty()
                        && track.name.len() <= 256
                        && track.insert_effects.len() <= 128
                        && track
                            .insert_effects
                            .iter()
                            .all(|effect| !effect.trim().is_empty() && effect.len() <= 256)
                })
        }) {
            anyhow::bail!("template contents are invalid");
        }
    ...
    pub fn remove_template(&mut self, name: &str) -> bool {
        ...
        self.templates
            .retain(|template| !template.name.eq_ignore_ascii_case(name.trim()));
        before != self.templates.len()
    }
    pub fn template_names(&self) -> Vec<String> {
        let mut names: Vec<_> = self.templates.iter().map(|t| t.name.clone()).collect();
        names.sort_by_key(|n| n.to_ascii_lowercase());
        names
    }
    pub fn rename_template(&mut self, old: &str, new: &str) -> bool {
        let n = new.trim();
        if n.is_empty()
            || n.len() > 256
            || n.contains('\0')
            || self
                .templates
                .iter()
                .any(|t| t.name.eq_ignore_ascii_case(n) && !t.name.eq_ignore_ascii_case(old))
        {
            return false;
        }
        let Some(t) = self
            .templates
            .iter_mut()
            .find(|t| t.name.eq_ignore_ascii_case(old.trim()))
        else {
            return false;
        };
        t.name = n.into();
    ...
    pub fn track_preset(&self, template: &str, track: &str) -> Option<TrackDef> {
        self.templates
            .iter()
            .find(|t| t.name.eq_ignore_ascii_case(template.trim()))?
            .tracks
            .iter()
            .find(|t| t.name.eq_ignore_ascii_case(track.trim()))
            .cloned()
    }
    pub fn track_names(&self, template: &str) -> Vec<String> {
        let Some(t) = self
            .templates
            .iter()
            .find(|t| t.name.eq_ignore_ascii_case(template.trim()))
        ...
        let mut names: Vec<_> = t.tracks.iter().map(|x| x.name.clone()).collect();
        names.sort_by_key(|n| n.to_ascii_lowercase());
        names
    }
    ...
    pub fn apply_track_preset(&mut self, template: &str, preset: TrackDef) -> bool {
        if preset.name.trim().is_empty()
            ...
            || preset.insert_effects.iter().any(|effect| {
                effect.trim().is_empty() || effect.len() > 256 || effect.contains('\0')
            })
        {
            return false;
        }
        ...
        if let Some(existing) = target
            .tracks
            .iter_mut()
            .find(|t| t.name.eq_ignore_ascii_case(&preset.name))
        {
            *existing = preset;
    ...
    pub fn audit_templates(&self) -> bool {
        !self.templates.is_empty()
            ...
            && self.templates.iter().enumerate().all(|(i, template)| {
                self.templates[..i]
                    .iter()
                    .all(|previous| !previous.name.eq_ignore_ascii_case(&template.name))
            })
            && self.templates.iter().all(|template| {
                !template.name.trim().is_empty()
                    ...
                    && template.tracks.iter().all(|track| {
                        !track.name.trim().is_empty()
                            && track.name.len() <= 256
                            && track.insert_effects.len() <= 128
                            && track
                    ...
                    && template.tracks.iter().enumerate().all(|(i, track)| {
                        template.tracks[..i]
                            .iter()
                            .all(|previous| !previous.name.eq_ignore_ascii_case(&track.name))
                    })
            })
    }
}
