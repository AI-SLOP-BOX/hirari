...
impl PhaseCoherentWarpGroup {
    pub fn new(channels: usize, time_ratio: f64) -> Option<Self> {
        if channels == 0 || channels > 256 || !time_ratio.is_finite() || !(0.125..=8.0).contains(&time_ratio) { return None; }
        Some(Self { channels, time_ratio })
    }
    ...
    pub fn process(&self, inputs: &[Vec<f32>], output_len: usize) -> Option<Vec<Vec<f32>>> {
        if inputs.len() != self.channels || inputs.iter().any(|track| track.is_empty() || track.len() > 16_000_000 || track.len() != inputs[0].len()) || output_len == 0 || output_len > 16_000_000 { return None; }
        ...
        for (index, tracks) in output.iter_mut().enumerate() {
            for (frame, sample) in tracks.iter_mut().enumerate() {
                ...
                if position >= (source_len - 1) as f64 { *sample = inputs[index][source_len - 1]; continue; }
                let base = position.floor() as usize;
                let fraction = (position - base as f64) as f32;
                let a = inputs[index][base];
                let b = inputs[index][base + 1];
    ...
    pub fn process_with_phase_offsets(&self, inputs: &[Vec<f32>], offsets: &[i32], output_len: usize) -> Option<Vec<Vec<f32>>> {
        if inputs.len() != self.channels || offsets.len() != self.channels || inputs.iter().any(|track| track.is_empty() || track.len() > 16_000_000 || track.len() != inputs[0].len()) || output_len == 0 || output_len > 16_000_000 || offsets.iter().any(|offset| offset.unsigned_abs() > 8192) { return None; }
        let source_len = inputs[0].len();
        let mut output = vec![vec![0.0; output_len]; self.channels];
        for (channel, track_output) in output.iter_mut().enumerate() {
            for (frame, sample) in track_output.iter_mut().enumerate() {
                ...
                if !(0.0..source_len as f64).contains(&base_position) { *sample = 0.0; continue; }
                let position = base_position;
                let base = position.floor() as usize;
                let fraction = (position - base as f64) as f32;
                let a = inputs[channel][base];
                let b = inputs[channel][(base + 1).min(source_len - 1)];
                *sample = if a.is_finite() && b.is_finite() { a + (b - a) * fraction } else { 0.0 };
            }
        }
        Some(output)
    ...
    pub fn estimate_phase_offsets(&self, inputs: &[Vec<f32>], max_offset: usize) -> Option<Vec<i32>> {
        if inputs.len() != self.channels || inputs.is_empty() || max_offset > 8192
            || inputs.iter().any(|track| track.is_empty() || track.len() != inputs[0].len() || track.len() > 16_000_000) { return None; }
        ...
        for (channel, track) in inputs.iter().enumerate().skip(1) {
            ...
            for signed in -(max_offset as i32)..=(max_offset as i32) {
                let (start_ref, start_track, count) = if signed >= 0 {
                    (signed as usize, 0usize, reference.len().saturating_sub(signed as usize))
                } else {
                    (0usize, signed.unsigned_abs() as usize, reference.len().saturating_sub(signed.unsigned_abs() as usize))
                };
                if count == 0 { continue; }
                let mut dot = 0.0f64; let mut ref_energy = 0.0f64; let mut track_energy = 0.0f64;
                let stride = if count < 4096 { 1 } else { 4 };
                for index in (0..count).step_by(stride) {
                    let a = reference[start_ref + index]; let b = track[start_track + index];
                    if !a.is_finite() || !b.is_finite() { continue; }
                    dot += f64::from(a) * f64::from(b); ref_energy += f64::from(a) * f64::from(a); track_energy += f64::from(b) * f64::from(b);
                }
                let denominator = (ref_energy * track_energy).sqrt();
                let score = if denominator > f64::EPSILON { dot / denominator } else { f64::NEG_INFINITY };
                if score > best.1 { best = (signed, score); }
            }
    ...
    pub fn validate(&self) -> bool { self.channels > 0 && self.channels <= 256 && self.time_ratio.is_finite() && (0.125..=8.0).contains(&self.time_ratio) }
}
...
impl ElasticWarpEngine {
    ...
    pub fn find_best_match(
        ...
        for test_pos in (start..end).step_by(2) {
            ...
            for j in (0..self.grain_size / 8).step_by(4) {
                let s0 = in_l[search_start + j] + in_r[search_start + j];
                let s1 = in_l[test_pos + j] + in_r[test_pos + j];
                corr += s0 * s1;
            }
    ...
    pub fn process_warp(
        ...
        for i in 0..num_samples {
            ...
            // 4. OUTPUT & OLA CLEAR
            out_l[i] = self.ola_buffer_l[self.write_idx];
            self.ola_buffer_l[self.write_idx] = 0.0;
            out_r[i] = self.ola_buffer_r[self.write_idx];
...
mod tests {
    ...
    fn group_warp_uses_one_time_map_for_all_microphones() {
        ...
        let output = group.process(&[vec![0.0, 1.0, 2.0, 3.0], vec![10.0, 11.0, 12.0, 13.0]], 4).unwrap();
        assert_eq!(output[0], vec![0.0, 0.5, 1.0, 1.5]);
        assert_eq!(output[1], vec![10.0, 10.5, 11.0, 11.5]);
    ...
    fn phase_offset_estimation_finds_shifted_transient() {
        ...
        let offsets = group.estimate_phase_offsets(&[reference, delayed], 2).unwrap();
    ...
    fn phase_offsets_are_applied_during_group_render() {
        let group = PhaseCoherentWarpGroup::new(2, 1.0).unwrap();
        let output = group.process_with_phase_offsets(
