    ...
    pub fn validate(&self) -> bool {
        ...
        total == self.total_bytes
            && self
                .missing
                .iter()
                .all(|path| !path.trim().is_empty() && missing.insert(path) && !sources.contains(path))
    }
}
...
impl ProjectArchive {
    ...
    pub fn plan(project_file: impl AsRef<Path>, project: &ProjectDocument) -> Result<ArchiveManifest> {
        ...
        for region in &project.regions {
            if !region.path.trim().is_empty() { paths.insert(resolve_path(project_root, &region.path)); }
        }
        for track in &project.tracks {
            for path in &track.sandbox_plugin_paths {
                if !path.trim().is_empty() { paths.insert(resolve_path(project_root, path)); }
            }
        }
        if paths.len() > MAX_DEPENDENCIES { bail!("project has too many dependencies"); }
        ...
        for source in paths {
            ...
            let metadata = match fs::metadata(&source) {
                ...
                _ => { missing.push(source_text); continue; }
            };
            if metadata.len() > MAX_FILE_BYTES { bail!("dependency is too large: {}", source.display()); }
            let hash = sha256_file(&source)?;
            let base = source.file_name().and_then(|name| name.to_str()).filter(|name| !name.is_empty()).unwrap_or("asset");
            let package_path = unique_package_path(base, &hash, &mut used);
            total_bytes = total_bytes.checked_add(metadata.len()).context("archive size overflow")?;
            entries.push(ArchiveEntry { source: source_text, package_path, bytes: metadata.len(), sha256: hash, exists: true });
        }
        let manifest = ArchiveManifest { schema_version: ARCHIVE_SCHEMA_VERSION, project_file: project_file.to_string_lossy().into_owned(), entries, missing, total_bytes };
        if !manifest.validate() { bail!("generated archive manifest is invalid"); }
        Ok(manifest)
    }
    ...
    pub fn create(project_file: impl AsRef<Path>, project_json: &[u8], project: &ProjectDocument, destination: impl AsRef<Path>) -> Result<ArchiveManifest> {
        let destination = destination.as_ref();
        if destination.exists() { bail!("archive destination already exists: {}", destination.display()); }
        let manifest = Self::plan(&project_file, project)?;
        let temp = destination.with_extension("aura-package.tmp");
        if temp.exists() { bail!("temporary archive destination already exists"); }
        ...
        let result = (|| -> Result<()> {
            ...
            for entry in &manifest.entries {
                ...
                fs::copy(&entry.source, &target).with_context(|| format!("copying {}", entry.source))?;
            ...
            atomic_write(&temp.join("manifest.json"), &serde_json::to_vec_pretty(&manifest)?)?;
    ...
    pub fn read_manifest(package: impl AsRef<Path>) -> Result<ArchiveManifest> {
        let bytes = fs::read(package.as_ref().join("manifest.json"))?;
        let manifest: ArchiveManifest = serde_json::from_slice(&bytes).context("invalid archive manifest")?;
        if !manifest.validate() { bail!("archive manifest failed validation"); }
        Ok(manifest)
    }
    ...
    pub fn verify(package: impl AsRef<Path>) -> Result<Vec<String>> {
        ...
        for entry in &manifest.entries {
            ...
            if !path.is_file() { errors.push(format!("missing packaged file: {}", entry.package_path)); continue; }
            let metadata = fs::metadata(&path)?;
            if metadata.len() != entry.bytes {
                errors.push(format!("size mismatch: {}", entry.package_path));
                continue;
            }
            if sha256_file(&path)? != entry.sha256 { errors.push(format!("checksum mismatch: {}", entry.package_path)); }
        }
        Ok(errors)
    }
    ...
    pub fn restore(package: impl AsRef<Path>, destination: impl AsRef<Path>) -> Result<Vec<PathBuf>> {
        let package = package.as_ref();
        let destination = destination.as_ref();
        if destination.exists() { bail!("restore destination already exists: {}", destination.display()); }
        if !Self::verify(package)?.is_empty() { bail!("archive verification failed"); }
        let manifest = Self::read_manifest(package)?;
        let temp = destination.with_extension("aura-restore.tmp");
        if temp.exists() { bail!("temporary restore destination already exists"); }
        fs::create_dir_all(&temp)?;
        let result = (|| -> Result<Vec<PathBuf>> {
            let project_source = package.join("project.json");
            if !project_source.is_file() { bail!("archive project.json is missing"); }
...
fn unique_package_path(base: &str, hash: &str, used: &mut BTreeMap<String, String>) -> String {
    let stem = Path::new(base).file_stem().and_then(|v| v.to_str()).unwrap_or("asset");
    let ext = Path::new(base).extension().and_then(|v| v.to_str()).map(|v| format!(".{v}")).unwrap_or_default();
...
mod tests {
    ...
    fn project(path: &str) -> ProjectDocument {
        ProjectDocument { schema_version: 1, contract_version: 1, project_id: "p".into(), metadata: ProjectMetadata::default(), sample_rate: 48_000.0, master_gain: 1.0, cycle_start_sample: 0, cycle_end_sample: 0, cycle_enabled: false, metronome_enabled: false, tracks: vec![], aux_track_ids: vec![], regions: vec![ProjectRegion { id: 1, track_id: 1, name: "a".into(), path: path.into(), start: 0, length: 1, source_offset: 0, base_source_offset: 0, base_length: 1, muted: false, clip_gain: 1.0, fade_in_samples: 0, fade_out_samples: 0, warp_ratio: 1.0, pitch_semitones: 0.0, reverse: false, loop_count: 1 }], plugin_instances: vec![], midi_learn_mappings: vec![], midi_notes: vec![], chord_track: vec![], midi_events: vec![], tempo_events: vec![], time_signature_events: vec![], macro_mappings: vec![], warp_markers: vec![], render_targets: vec![], freeze_artifacts: vec![], sidechain_routes: vec![], feedback_routes: vec![], audio_routes: vec![], openutau_vocals: vec![], comp_takes: vec![], comp_segments: vec![], track_stacks: vec![], markers: vec![], vca_groups: vec![], hardware_inserts: vec![] }
    }
    ...
    fn plans_missing_and_existing_dependencies() {
        ...
        assert!(manifest.validate()); assert_eq!(manifest.entries.len(), 1); assert!(manifest.missing.is_empty());
    ...
    fn creates_and_verifies_package() {
        ...
        let _ = fs::remove_dir_all(&root); fs::create_dir_all(&root).unwrap(); fs::write(root.join("take.wav"), b"audio").unwrap();
        let package = root.join("song.aura-package");
        let manifest = ProjectArchive::create(root.join("song.aura"), b"{}", &project("take.wav"), &package).unwrap();
        assert_eq!(ProjectArchive::verify(&package).unwrap(), Vec::<String>::new()); assert!(manifest.validate());
        let _ = fs::remove_dir_all(root);
    }

    #[test]
    fn gives_same_named_dependencies_unique_package_paths() {
        let root = std::env::temp_dir().join(format!("aura-archive-collision-{}", std::process::id()));
        let _ = fs::remove_dir_all(&root);
        ...
        document.regions.push(ProjectRegion { id: 2, path: "two/take.wav".into(), ..document.regions[0].clone() });

        let manifest = ProjectArchive::plan(root.join("song.aura"), &document).unwrap();

        assert!(manifest.validate());
        assert_eq!(manifest.entries.len(), 2);
        assert_ne!(manifest.entries[0].package_path, manifest.entries[1].package_path);
        let _ = fs::remove_dir_all(root);
    }
    ...
    fn verify_reports_packaged_file_size_changes() {
        ...
        let package = root.join("song.aura-package");
        let manifest = ProjectArchive::create(root.join("song.aura"), b"{}", &project("take.wav"), &package).unwrap();
        fs::write(package.join(&manifest.entries[0].package_path), b"longer audio").unwrap();

        let errors = ProjectArchive::verify(&package).unwrap();

        assert_eq!(errors, vec![format!("size mismatch: {}", manifest.entries[0].package_path)]);
        let _ = fs::remove_dir_all(root);
    }
    ...
    fn restores_verified_package_atomically() {
        ...
        let _ = fs::remove_dir_all(&root); fs::create_dir_all(&root).unwrap(); fs::write(root.join("take.wav"), b"audio").unwrap();
        let package = root.join("song.aura-package");
        ProjectArchive::create(root.join("song.aura"), b"{}", &project("take.wav"), &package).unwrap();
        let destination = root.join("restored");
        let restored = ProjectArchive::restore(&package, &destination).unwrap();
        assert_eq!(restored.len(), 2);
        assert_eq!(fs::read(destination.join("Media/take.wav")).unwrap(), b"audio");
        let _ = fs::remove_dir_all(root);
    }
}
