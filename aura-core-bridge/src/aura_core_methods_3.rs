impl AuraCore {
    pub fn push_midi_monitor_event(&self, timestamp: u64, status: u8, data1: u8, data2: u8) -> bool {
        if status & 0x80 == 0 || data1 > 127 || data2 > 127 { return false; }
        self.midi_monitor.lock().map(|mut m| m.push(crate::midi_monitor::MidiMonitorEvent { timestamp, status, data1, data2 })).is_ok()
    }
    pub fn clear_midi_monitor(&self) { if let Ok(mut m) = self.midi_monitor.lock() { m.clear(); } }
    pub fn midi_monitor_json(&self, count: usize) -> String { self.midi_monitor.lock().map(|m| serde_json::json!({"ok":true,"events":m.snapshot(count.min(4096))}).to_string()).unwrap_or_else(|_| "{\"ok\":false}".to_owned()) }
    ...
    pub fn add_chord_event(&self, tick: u64, root: u8, intervals: Vec<u8>, name: &str) -> bool {
        ...
        let before = self.chord_track.lock().map(|track| track.clone()).unwrap_or_default();
        let Ok(mut track) = self.chord_track.lock() else { return false; };
        ...
        if let Ok(mut history) = self.chord_history.lock() {
            history.push(crate::ChordHistoryEntry { depth_after: 0, before, after });
        }
    ...
    pub fn chord_track_json(&self) -> String {
        let track = self.chord_track.lock().map(|events| events.clone()).unwrap_or_default();
        serde_json::to_string(&track).unwrap_or_else(|_| "[]".to_owned())
    }

    fn record_chord_snapshot(&self, before: Vec<crate::harmonic::ChordEvent>, after: Vec<crate::harmonic::ChordEvent>) {
    ...
    pub fn remove_chord_events_range(&self, start_tick: u64, end_tick: u64) -> usize {
        let Ok(mut track) = self.chord_track.lock() else { return 0; };
        ...
        old_len.saturating_sub(self.chord_track.lock().map(|track| track.len()).unwrap_or(old_len))
    }
    ...
    {
        if tid == 0 || rid == 0 {
            let result = crate::bridge_error::BridgeError::new("invalid_region_target", "track and region ids must be non-zero")
                .object(format!("track:{tid}/region:{rid}"))
                .at_generation(self.project_generation());
            return serde_json::to_string(&result).unwrap_or_else(|_| "{\"code\":\"diagnostic_serialization_failed\"}".to_owned());
        }
        if !valid {
            let result = crate::bridge_error::BridgeError::new("invalid_region_value", format!("{op} received an invalid value"))
                .object(format!("track:{tid}/region:{rid}"))
                .at_generation(self.project_generation());
            return serde_json::to_string(&result).unwrap_or_else(|_| "{\"code\":\"diagnostic_serialization_failed\"}".to_owned());
        }
        ...
        let result = if apply(engine) {
            return format!("{{\"ok\":true,\"operation\":\"{op}\",\"track_id\":{tid},\"region_id\":{rid}}}");
        } else {
            crate::bridge_error::BridgeError::new("region_not_found_or_rejected", format!("{op} was rejected"))
                .object(format!("track:{tid}/region:{rid}"))
                .at_generation(self.project_generation())
        };
    ...
    pub fn move_region_diagnostic_json(&self, tid: u32, rid: u32, start: f64) -> String {
        self.region_diagnostic("move_region", tid, rid, start.is_finite() && start >= 0.0, |e| {
            e.move_region(tid, rid, self.beats_to_samples(start) as f64)
        })
    }

    pub fn split_region_diagnostic_json(&self, tid: u32, rid: u32, beat: f64) -> String {
        self.region_diagnostic("split_region", tid, rid, beat.is_finite() && beat > 0.0, |e| {
            e.split_region(tid, rid, self.beats_to_samples(beat) as f64)
        })
    }

    pub fn set_region_gain_diagnostic_json(&self, tid: u32, rid: u32, gain: f32) -> String {
        self.region_diagnostic("set_region_gain", tid, rid, gain.is_finite() && (-24.0..=24.0).contains(&gain), |e| e.set_region_gain(tid, rid, gain))
    }

    pub fn set_region_reverse_diagnostic_json(&self, tid: u32, rid: u32, reverse: bool) -> String {
        self.region_diagnostic("set_region_reverse", tid, rid, true, |e| e.set_region_reverse(tid, rid, reverse))
    }

    pub fn set_region_muted_diagnostic_json(&self, tid: u32, rid: u32, muted: bool) -> String {
        self.region_diagnostic("set_region_muted", tid, rid, true, |e| e.set_region_muted(tid, rid, muted))
    }

    pub fn set_region_fades_diagnostic_json(&self, tid: u32, rid: u32, fade_in: f32, fade_out: f32) -> String {
        let valid = fade_in.is_finite() && fade_out.is_finite() && (0.0..=1.0).contains(&fade_in) && (0.0..=1.0).contains(&fade_out);
        self.region_diagnostic("set_region_fades", tid, rid, valid, |e| e.set_region_fades(tid, rid, fade_in, fade_out))
    }

    pub fn set_region_trim_diagnostic_json(&self, tid: u32, rid: u32, start: f32, end: f32) -> String {
        let valid = start.is_finite() && end.is_finite() && (0.0..=1.0).contains(&start) && (0.0..=1.0).contains(&end) && start < end;
        self.region_diagnostic("set_region_trim", tid, rid, valid, |e| e.set_region_trim(tid, rid, start, end))
    }

    pub fn set_region_warp_ratio_diagnostic_json(&self, tid: u32, rid: u32, ratio: f64) -> String {
        self.region_diagnostic("set_region_warp_ratio", tid, rid, ratio.is_finite() && (0.25..=4.0).contains(&ratio), |e| e.set_region_warp_ratio(tid, rid, ratio))
    }

    pub fn set_region_pitch_diagnostic_json(&self, tid: u32, rid: u32, semitones: f32) -> String {
        self.region_diagnostic("set_region_pitch", tid, rid, semitones.is_finite() && (-48.0..=48.0).contains(&semitones), |e| e.set_region_pitch_semitones(tid, rid, semitones))
    }

    pub fn set_region_loop_diagnostic_json(&self, tid: u32, rid: u32, count: u32) -> String {
        self.region_diagnostic("set_region_loop_count", tid, rid, (1..=1024).contains(&count), |e| e.set_region_loop_count(tid, rid, count))
    }

    pub fn request_video_frame_diagnostic_json(&self, seconds: f64) -> String {
        if !seconds.is_finite() || seconds < 0.0 {
            let result = crate::bridge_error::BridgeError::new("invalid_video_position", "video position must be finite and non-negative");
            return serde_json::to_string(&result).unwrap_or_else(|_| "{\"code\":\"diagnostic_serialization_failed\"}".to_owned());
        }
        let Some(engine) = self.engine.as_ref() else {
            return "{\"code\":\"engine_unavailable\",\"retryable\":true}".to_owned();
        ...
        let result = if engine.request_video_frame(seconds) {
            return format!("{{\"ok\":true,\"operation\":\"request_video_frame\",\"seconds\":{seconds}}}");
        } else {
            crate::bridge_error::BridgeError::new("video_request_rejected", "video frame request was rejected")
        };
        serde_json::to_string(&result).unwrap_or_else(|_| "{\"code\":\"diagnostic_serialization_failed\"}".to_owned())
    }
    ...
    pub fn load_video_diagnostic_json(&self, path: &str) -> String {
        ...
        if path.as_os_str().is_empty() || path.to_string_lossy().contains('\0') {
            let result = crate::bridge_error::BridgeError::new("invalid_video_path", "video path is empty or invalid");
            return serde_json::to_string(&result).unwrap_or_else(|_| "{\"code\":\"diagnostic_serialization_failed\"}".to_owned());
        }
        if !path.is_file() {
            let result = crate::bridge_error::BridgeError::new("video_not_found", "video path does not refer to a regular file").retryable(true);
            return serde_json::to_string(&result).unwrap_or_else(|_| "{\"code\":\"diagnostic_serialization_failed\"}".to_owned());
        }
        let Some(engine) = self.engine.as_ref() else {
            return "{\"code\":\"engine_unavailable\",\"retryable\":true}".to_owned();
        ...
        let result = if engine.load_video(path.to_string_lossy().as_ref()) {
            return format!("{{\"ok\":true,\"operation\":\"load_video\"}}");
        } else {
            crate::bridge_error::BridgeError::new("video_load_rejected", "video loading was rejected")
        };
        serde_json::to_string(&result).unwrap_or_else(|_| "{\"code\":\"diagnostic_serialization_failed\"}".to_owned())
    }
    ...
    {
        if tid == 0 || !valid {
            let result = crate::bridge_error::BridgeError::new("invalid_track_command", format!("{op} received an invalid target or value"))
        ...
        let result = if apply(engine) {
            ...
            crate::bridge_error::BridgeError::new("track_not_found_or_rejected", format!("{op} was rejected"))
                .object(format!("track:{tid}"))
                .at_generation(self.project_generation())
        };
        serde_json::to_string(&result).unwrap_or_else(|_| "{\"code\":\"diagnostic_serialization_failed\"}".to_owned())
    ...
    pub fn set_spatial_position_diagnostic_json(&self, tid: u32, x: f32, y: f32, z: f32) -> String {
        let valid = x.is_finite() && y.is_finite() && z.is_finite() && (-1.0..=1.0).contains(&x) && (-1.0..=1.0).contains(&y) && (-1.0..=1.0).contains(&z);
        self.track_diagnostic("set_spatial_position", tid, valid, |e| e.set_spatial_position(tid, x, y, z))
    }

    pub fn set_track_armed_diagnostic_json(&self, tid: u32, armed: bool) -> String {
        self.track_diagnostic("set_track_armed", tid, true, |e| e.set_track_armed(tid, armed))
    }

    pub fn set_phase_invert_diagnostic_json(&self, tid: u32, inverted: bool) -> String {
        self.track_diagnostic("set_phase_invert", tid, true, |e| e.set_phase_invert(tid, inverted))
    }

    pub fn set_track_eq_diagnostic_json(&self, tid: u32, low_gain: f32, low_cut: f32, high_gain: f32, high_cut: f32) -> String {
        let valid = [low_gain, low_cut, high_gain, high_cut].iter().all(|value| value.is_finite())
            && (-24.0..=24.0).contains(&low_gain)
            && (0.0..=1.0).contains(&low_cut)
            && (-24.0..=24.0).contains(&high_gain)
            && (0.0..=1.0).contains(&high_cut);
        if tid == 0 || !valid {
            return serde_json::to_string(&crate::bridge_error::BridgeError::new("invalid_track_command", "set_track_eq received an invalid target or value")
                .object(format!("track:{tid}"))
                .at_generation(self.project_generation()))
                .unwrap_or_else(|_| "{\"code\":\"diagnostic_serialization_failed\"}".to_owned());
        }
        let Some(engine) = self.engine.as_ref() else {
            return serde_json::to_string(&crate::bridge_error::BridgeError::new("engine_unavailable", "audio engine unavailable").retryable(true))
                .unwrap_or_else(|_| "{\"code\":\"diagnostic_serialization_failed\"}".to_owned());
        };
        if engine.set_track_eq(tid, low_gain, low_cut, high_gain, high_cut) {
            format!("{{\"ok\":true,\"operation\":\"set_track_eq\",\"track_id\":{tid}}}")
        } else {
            serde_json::to_string(&crate::bridge_error::BridgeError::new("track_not_found_or_rejected", "set_track_eq was rejected")
                .object(format!("track:{tid}"))
                .at_generation(self.project_generation()))
                .unwrap_or_else(|_| "{\"code\":\"diagnostic_serialization_failed\"}".to_owned())
        }
...
pub fn split_region(&self, tid: u32, rid: u32, beat: f64) -> bool {
    ...
    pub fn duplicate_region(&self, tid: u32, rid: u32, start: f64) -> u32 {
        ...
        self.engine
            .as_ref()
            .map_or(0, |e| e.duplicate_region(tid, rid, self.beats_to_samples(start)))
    }
    pub fn remove_region(&self, tid: u32, rid: u32) -> bool {
        if tid == 0 || rid == 0 {
            return false;
        }
        self.engine.as_ref().is_some_and(|e| e.remove_region(tid, rid))
    }
    ...
    pub fn analyze_region_audio_note_segments_diagnostic_json(
        &self, tid: u32, rid: u32, sample_rate: f64,
    ) -> String {
    ...
    pub fn remove_midi_notes_range_diagnostic_json(
        ...
        if let Ok(mut notes) = self.scheduled_midi_notes.lock() {
            notes.retain(|note| {
                ...
                !(note.track_id == track_id && note.start_sample < end_sample && note_end > start_sample)
            });
        }
        serde_json::json!({
            "ok": true,
    ...
    pub fn transpose_midi_notes_range_diagnostic_json(
        ...
        if track_id == 0 || start_sample >= end_sample || !(-127..=127).contains(&semitones) {
            return "{\"ok\":false,\"code\":\"invalid_midi_transpose\",\"retryable\":false}".to_owned();
        }
        let notes = match self.scheduled_midi_notes.lock() {
            Ok(notes) => notes,
            Err(_) => {
                return "{\"ok\":false,\"code\":\"midi_state_unavailable\",\"retryable\":true}".to_owned();
            }
        };
        {
    ...
    pub fn move_midi_notes_range_diagnostic_json(
        ...
        if track_id == 0 || start_sample >= end_sample {
            return "{\"ok\":false,\"code\":\"invalid_midi_move_range\",\"retryable\":false}".to_owned();
        }
        let notes = match self.scheduled_midi_notes.lock() {
            Ok(notes) => notes,
            Err(_) => {
        ...
        if let Ok(mut notes) = self.scheduled_midi_notes.lock() {
            for note in notes.iter_mut().filter(|note| {
                ...
                note.start_sample = if delta_samples >= 0 {
                    ...
                    note.start_sample.saturating_sub(delta_samples.unsigned_abs())
                };
            }
        }
        serde_json::json!({
    ...
    pub fn set_midi_note(
        ...
        {
            if let Ok(mut notes) = self.scheduled_midi_notes.lock() {
                let lyric = notes.iter()
                    .find(|existing| existing.track_id == track_id
                        && existing.pitch == pitch
                        && existing.start_sample == start_sample)
                    .map(|existing| existing.lyric.clone())
                    .unwrap_or_default();
                notes.retain(|existing| !(existing.track_id == track_id
                    && existing.pitch == pitch
                    && existing.start_sample == start_sample));
                let note = crate::project_contracts::MidiNoteContract {
                    track_id, pitch, velocity, start_sample, length_samples,
                    lyric,
                    phoneme: String::new(), pitch_curve_cents: Vec::new(),
                    vibrato_depth_cents: 0, portamento_samples: 0, probability: 100, repeat_count: 1,
                };
    ...
    pub fn set_midi_note_lyric_diagnostic_json(
        ...
        if pitch > 127 || velocity == 0 || length_samples == 0
            || start_sample.checked_add(length_samples).is_none()
            || lyric.len() > 1_024 || lyric.contains('\0') {
            return serde_json::json!({
                "ok": false,
    ...
    pub fn set_midi_note_lyric(
        ...
        let before_metadata = self.scheduled_midi_notes
            .lock()
        ...
        notes.retain(|note| !(note.track_id == track_id
            && note.pitch == pitch
    ...
    pub fn set_midi_note_articulation(
        ...
        if pitch > 127 || phoneme.len() > 128 || phoneme.contains('\0')
            || pitch_curve_cents.len() > 256 {
            return false;
        }
        let Ok(mut notes) = self.scheduled_midi_notes.lock() else { return false; };
        ...
        note.portamento_samples = portamento_samples.min(note.length_samples.min(u64::from(u32::MAX)) as u32);
        ...
        if let Some(engine) = self.engine.as_ref() {
            ...
            if let Ok(mut history) = self.midi_lyric_history.lock() {
                if let Some(entry) = history.iter_mut().rev().find(|entry| entry.depth_after == depth) {
    ...
    pub fn replace_midi_note_contracts(
        ...
        if !engine.replace_midi_notes(packed, record_undo) { return false; }
    ...
    fn sync_midi_note_metadata_from_engine(&self) {
        ...
        for chunk in packed.chunks_exact(5) {
            ...
            synced.push(crate::project_contracts::MidiNoteContract {
                ...
                phoneme: previous.map(|note| note.phoneme.clone()).unwrap_or_default(),
                pitch_curve_cents: previous.map(|note| note.pitch_curve_cents.clone()).unwrap_or_default(),
                vibrato_depth_cents: previous.map(|note| note.vibrato_depth_cents).unwrap_or_default(),
                portamento_samples: previous.map(|note| note.portamento_samples).unwrap_or_default(),
    ...
    pub fn midi_notes_json(&self) -> String {
        let mut notes = self.scheduled_midi_notes
            .lock()
            .map(|notes| notes.clone())
            .unwrap_or_default();
        notes.sort_by_key(|note| (note.track_id, note.start_sample, note.pitch));
        let enriched = notes.into_iter().map(|note| {
            let mut value = serde_json::to_value(&note).unwrap_or_else(|_| serde_json::json!({}));
            if let Some(object) = value.as_object_mut() {
                object.insert(
                    "drum_lane".to_owned(),
                    serde_json::Value::String(crate::piano_roll_editor::drum_lane_label(note.pitch).to_owned()),
                );
            }
            value
        }).collect::<Vec<_>>();
    ...
    pub fn set_mute_diagnostic_json(&self, tid: u32, m: bool) -> String {
        self.set_track_toggle_diagnostic_json("mute", tid, |engine, id| engine.set_track_mute(id, m))
    }
    ...
    pub fn set_solo_diagnostic_json(&self, tid: u32, s: bool) -> String {
        self.set_track_toggle_diagnostic_json("solo", tid, |engine, id| engine.set_track_solo(id, s))
    }
    ...
    pub fn execute_vocal_remover_diagnostic_json(&self, tid: u32) -> String {
        if tid == 0 {
            return serde_json::to_string(&crate::bridge_error::BridgeError::new(
                "invalid_track_id", "track id must be non-zero",
            ).object(format!("track:{tid}")))
            .unwrap_or_else(|_| "{\"code\":\"diagnostic_serialization_failed\"}".to_owned());
        }
        let Some(engine) = self.engine.as_ref() else {
        ...
        if engine.execute_vocal_remover(tid) {
            return format!("{{\"ok\":true,\"operation\":\"execute_vocal_remover\",\"track_id\":{tid}}}");
        }
        serde_json::to_string(&crate::bridge_error::BridgeError::new(
            "track_not_found_or_rejected", "vocal remover requires an existing stereo track",
        ).object(format!("track:{tid}")).at_generation(self.project_generation()))
        .unwrap_or_else(|_| "{\"code\":\"diagnostic_serialization_failed\"}".to_owned())
    }
    ...
    pub fn set_articulation_map_diagnostic_json(&self, tid: u32, map: &str) -> String {
        if tid == 0 || map.trim().is_empty() || map.contains('\0') {
            return serde_json::to_string(&crate::bridge_error::BridgeError::new(
                "invalid_articulation_map", "track id or articulation map is invalid",
            ).object(format!("track:{tid}")))
            .unwrap_or_else(|_| "{\"code\":\"diagnostic_serialization_failed\"}".to_owned());
        }
        let mut hash = 2166136261u32;
        ...
        if engine.set_articulation_map(tid, hash) {
            return format!("{{\"ok\":true,\"operation\":\"set_articulation_map\",\"track_id\":{tid}}}");
        }
        serde_json::to_string(&crate::bridge_error::BridgeError::new(
            "track_not_found_or_rejected", "articulation map target was rejected",
        ).object(format!("track:{tid}")).at_generation(self.project_generation()))
        .unwrap_or_else(|_| "{\"code\":\"diagnostic_serialization_failed\"}".to_owned())
    }
    ...
    pub fn undo(&self) {
        if let Some(e) = self.engine.as_ref() {
            if e.get_undo_count() == 0 {
                if let Ok(mut history) = self.chord_history.lock() {
                    if let Some(entry) = history.pop() {
                        if let Ok(mut chords) = self.chord_track.lock() { *chords = entry.before.clone(); }
                        if let Ok(mut redo) = self.chord_redo_history.lock() { redo.push(entry); }
                    }
                }
                if let Ok(mut history) = self.comping_history.lock() {
                    if let Some(entry) = history.pop() {
                        let _ = self.restore_comping_snapshot_json(&serde_json::to_string(&entry.before).unwrap_or_default());
                        if let Ok(mut redo) = self.comping_redo_history.lock() { redo.push(entry); }
                    }
                }
                return;
            }
            ...
            if let Ok(history) = self.chord_history.lock() {
                if let Some(entry) = history.iter().find(|entry| entry.depth_after == depth_before) {
                    if let Ok(mut chords) = self.chord_track.lock() { *chords = entry.before.clone(); }
                }
            }
    ...
    pub fn end_undo_transaction(&self) -> bool {
        self.engine.as_ref().is_some_and(|e| e.end_undo_transaction())
    }

    pub fn abort_undo_transaction(&self) -> bool {
        self.engine.as_ref().is_some_and(|e| e.abort_undo_transaction())
    }
    ...
    pub fn redo(&self) {
        if let Some(e) = self.engine.as_ref() {
            if e.get_redo_count() == 0 {
                if let Ok(mut redo) = self.chord_redo_history.lock() {
                    if let Some(entry) = redo.pop() {
                        if let Ok(mut chords) = self.chord_track.lock() { *chords = entry.after.clone(); }
                        if let Ok(mut history) = self.chord_history.lock() { history.push(entry); }
                    }
                }
                if let Ok(mut redo) = self.comping_redo_history.lock() {
                    if let Some(entry) = redo.pop() {
                        let _ = self.restore_comping_snapshot_json(&serde_json::to_string(&entry.after).unwrap_or_default());
                        if let Ok(mut history) = self.comping_history.lock() { history.push(entry); }
                    }
                }
                return;
            }
            ...
            if let Ok(history) = self.chord_history.lock() {
                if let Some(entry) = history.iter().find(|entry| entry.depth_after == depth_after) {
                    if let Ok(mut chords) = self.chord_track.lock() { *chords = entry.after.clone(); }
                }
            }
            if let Ok(history) = self.midi_lyric_history.lock() {
    ...
    pub fn undo_depth(&self) -> u32 {
        self.engine.as_ref().map_or(0, |engine| engine.get_undo_count())
    }

    pub fn redo_depth(&self) -> u32 {
        self.engine.as_ref().map_or(0, |engine| engine.get_redo_count())
    }
    ...
    pub fn start_render_diagnostic_json(&self, path: &str) -> String {
        if !is_wav_output_path(path) {
            let result = crate::bridge_error::BridgeError::new("invalid_render_path", "render output must be a supported WAV path");
            return serde_json::to_string(&result).unwrap_or_else(|_| "{\"code\":\"diagnostic_serialization_failed\"}".to_owned());
        }
        let Some(engine) = self.engine.as_ref() else {
            return "{\"code\":\"engine_unavailable\",\"retryable\":true}".to_owned();
        ...
        let result = if engine.bounce_project_async(path, 0) {
            return format!("{{\"ok\":true,\"operation\":\"start_render\",\"path\":{}}}", serde_json::to_string(path).unwrap_or_default());
        } else {
            crate::bridge_error::BridgeError::new("render_rejected", "render could not be scheduled").retryable(true)
        };
        serde_json::to_string(&result).unwrap_or_else(|_| "{\"code\":\"diagnostic_serialization_failed\"}".to_owned())
    }
    ...
    pub fn cancel_render_diagnostic_json(&self) -> String {
        ...
        let result = if engine.cancel_bounce() {
            ...
            crate::bridge_error::BridgeError::new("render_not_active", "no active render accepted cancellation")
        };
        serde_json::to_string(&result).unwrap_or_else(|_| "{\"code\":\"diagnostic_serialization_failed\"}".to_owned())
    }
    ...
    pub fn execute_auto_mixing_diagnostic_json(&self) -> String {
        ...
        serde_json::to_string(&crate::bridge_error::BridgeError::new(
            "project_empty_or_rejected", "auto mixing requires at least one track",
        ).at_generation(self.project_generation()))
        .unwrap_or_else(|_| "{\"code\":\"diagnostic_serialization_failed\"}".to_owned())
    }
    ...
    pub fn execute_auto_arrangement_diagnostic_json(&self) -> String {
        ...
        serde_json::to_string(&crate::bridge_error::BridgeError::new(
            "project_empty_or_rejected", "auto arrangement requires at least one track",
        ).at_generation(self.project_generation()))
        .unwrap_or_else(|_| "{\"code\":\"diagnostic_serialization_failed\"}".to_owned())
    }
    ...
    pub fn set_project_scale_diagnostic_json(&self, root: i32, scale_type: i32) -> String {
        if !(0..=11).contains(&root) || !(0..=32).contains(&scale_type) {
            return serde_json::to_string(&crate::bridge_error::BridgeError::new(
                "invalid_project_scale", "root or scale type is outside the supported range",
            ).at_generation(self.project_generation()))
            .unwrap_or_else(|_| "{\"code\":\"diagnostic_serialization_failed\"}".to_owned());
        }
        let Some(engine) = self.engine.as_ref() else {
        ...
        serde_json::to_string(&crate::bridge_error::BridgeError::new(
            "project_scale_rejected", "project scale was rejected by the native engine",
        ).at_generation(self.project_generation()))
        .unwrap_or_else(|_| "{\"code\":\"diagnostic_serialization_failed\"}".to_owned())
    }
    ...
    pub fn set_low_latency_mode(&self, active: bool) -> bool {
        self.engine.as_ref().is_some_and(|engine| engine.set_low_latency_mode(active))
    }

    pub fn low_latency_mode(&self) -> bool {
        self.engine.as_ref().is_some_and(|engine| engine.low_latency_mode())
    }

    pub fn set_tonal_scale(&self, root: i32, scale_type: u32) -> bool {
        self.engine.as_ref().is_some_and(|engine| engine.set_tonal_scale(root, scale_type))
    }

    pub fn is_note_in_tonal_scale(&self, midi_note: i32) -> bool {
        self.engine.as_ref().is_some_and(|engine| engine.is_note_in_tonal_scale(midi_note))
    }
    ...
    pub fn generate_arpeggio(
        ...
        if pitches.is_empty() || pitches.len() != velocities.len() || steps == 0 { return Vec::new(); }
        ...
        arp.update_notes(pitches.into_iter().zip(velocities).map(|(pitch, velocity)| crate::arpeggio::HeldNote { pitch, velocity }).collect());
        (0..steps).filter_map(|_| arp.get_next_note()).collect()
    }

    pub fn place_arpeggio(
        &self, track_id: u32, start_sample: u64, step_samples: u64, gate_samples: u64,
        pitches: Vec<u8>, velocities: Vec<u8>, pattern: u32, octaves: u32, steps: u32,
    ) -> usize {
        if step_samples == 0 || gate_samples == 0 || gate_samples > step_samples { return 0; }
        let generated = self.generate_arpeggio(pitches, velocities, pattern, octaves, steps);
        let Some(engine) = self.engine.as_ref() else { return 0; };
        let mut notes = self.scheduled_midi_notes.lock().map(|notes| notes.clone()).unwrap_or_default();
        for (index, (pitch, velocity)) in generated.iter().enumerate() {
            ...
            let lyric = notes.iter().find(|note| note.track_id == track_id && note.pitch == *pitch && note.start_sample == position).map(|note| note.lyric.clone()).unwrap_or_default();
            notes.retain(|note| !(note.track_id == track_id && note.pitch == *pitch && note.start_sample == position));
            notes.push(crate::project_contracts::MidiNoteContract { track_id, pitch: *pitch, velocity: *velocity, start_sample: position, length_samples: gate_samples, lyric, phoneme: String::new(), pitch_curve_cents: Vec::new(), vibrato_depth_cents: 0, portamento_samples: 0, probability: 100, repeat_count: 1 });
        }
        let mut packed = Vec::with_capacity(notes.len() * 5);
        for note in &notes { packed.extend([note.track_id as u64, note.pitch as u64, note.velocity as u64, note.start_sample, note.length_samples]); }
        if !engine.replace_midi_notes(packed, true) { return 0; }
        if let Ok(mut current) = self.scheduled_midi_notes.lock() { *current = notes; }
        generated.len()
    }
    ...
    pub fn place_generated_chord(
        ...
        if velocity == 0 || length_samples == 0 { return 0; }
        let pitches = self.generate_chord_notes(root, octave, quality);
        if pitches.is_empty() { return 0; }
        let Some(engine) = self.engine.as_ref() else { return 0; };
        let mut notes = self.scheduled_midi_notes.lock().map(|notes| notes.clone()).unwrap_or_default();
        for pitch in &pitches {
            let lyric = notes.iter().find(|note| note.track_id == track_id && note.pitch == *pitch && note.start_sample == start_sample).map(|note| note.lyric.clone()).unwrap_or_default();
            notes.retain(|note| !(note.track_id == track_id && note.pitch == *pitch && note.start_sample == start_sample));
            notes.push(crate::project_contracts::MidiNoteContract {
                track_id, pitch: *pitch, velocity, start_sample, length_samples, lyric,
                phoneme: String::new(), pitch_curve_cents: Vec::new(), vibrato_depth_cents: 0, portamento_samples: 0, probability: 100, repeat_count: 1,
            });
        }
        let mut packed = Vec::with_capacity(notes.len() * 5);
        for note in &notes {
            packed.extend([note.track_id as u64, note.pitch as u64, note.velocity as u64, note.start_sample, note.length_samples]);
        }
        if !engine.replace_midi_notes(packed, true) { return 0; }
        if let Ok(mut current) = self.scheduled_midi_notes.lock() { *current = notes; }
        pitches.len()
    ...
    pub fn save_project(&self, path: &str) -> bool {
        ...
        if had_primary {
            ...
            if copy_file_atomic_replace(std::path::Path::new(&backup), std::path::Path::new(path)) {
                let _ = self.engine.as_ref().is_some_and(|e| e.load_project(&backup));
            }
        } else {
...
pub fn split_region_at_silence(&self, tid: u32, rid: u32, samples: &[f32], threshold: f32, min_length: usize) -> u32 {
    if tid == 0 || rid == 0 || !threshold.is_finite() || !(0.0..=1.0).contains(&threshold) || min_length == 0 { return 0; }
    let Ok(layout) = serde_json::from_str::<serde_json::Value>(&self.get_project_layout_json()) else { return 0; };
    let Some(region) = layout.as_array().and_then(|tracks| tracks.iter().find_map(|track| track.get("regions").and_then(serde_json::Value::as_array).and_then(|regions| regions.iter().find(|region| region.get("id").and_then(serde_json::Value::as_u64) == Some(rid as u64))))) else { return 0; };
    let region_start = region.get("start").and_then(serde_json::Value::as_u64).unwrap_or(0);
    let region_length = region.get("length").and_then(serde_json::Value::as_u64).unwrap_or(samples.len() as u64);
    let mut boundaries = crate::silence_detector::detect_silence(samples, threshold, min_length).into_iter().flat_map(|range| { let start = range.start as u64; let end = start.saturating_add(range.length as u64); [start, end] }).filter(|offset| *offset > 0 && *offset < region_length).map(|offset| region_start.saturating_add(offset)).collect::<Vec<_>>();
    boundaries.sort_unstable();
    boundaries.dedup();
    let mut applied = 0;
    for sample in boundaries.into_iter().rev() {
        if self.split_region(tid, rid, self.samples_to_beats(sample)) { applied += 1; }
    }
    applied
}
    ...
    pub fn split_region_with_auto_crossfade(&self, tid: u32, rid: u32, beat: f64, ratio: f32) -> bool {
        if tid == 0 || rid == 0 || !beat.is_finite() || beat <= 0.0
            || !ratio.is_finite() || !(0.0..=1.0).contains(&ratio) { return false; }
        let split_samples = self.beats_to_samples(beat);
        if !self.split_region(tid, rid, beat) { return false; }
        let Ok(layout) = serde_json::from_str::<serde_json::Value>(&self.get_project_layout_json()) else { return false; };
        let Some(track) = layout.as_array().and_then(|tracks| tracks.iter().find(|track| track.get("id").and_then(serde_json::Value::as_u64) == Some(tid as u64))) else { return false; };
        let Some(regions) = track.get("regions").and_then(serde_json::Value::as_array) else { return false; };
        let right_id = regions.iter().find(|region| region.get("start").and_then(serde_json::Value::as_u64) == Some(split_samples)).and_then(|region| region.get("id").and_then(serde_json::Value::as_u64)).unwrap_or(0) as u32;
        right_id != 0 && self.set_region_fades(tid, right_id, ratio, 0.0) && self.set_region_fades(tid, rid, 0.0, ratio)
    }
    ...
    pub fn save_project_diagnostic_json(&self, path: &str) -> String {
        let result = if path.trim().is_empty() {
            Err(crate::bridge_error::BridgeError::new(
                "invalid_path", "project path must not be empty",
            ))
            ...
            Err(crate::bridge_error::BridgeError::new(
                "project_save_failed", "project or sidecar publication failed",
            )
            .retryable(true)
            .at_generation(self.project_generation()))
        };
    ...
    pub fn load_project(&self, path: &str) -> bool {
        ...
        for sidecar in [Self::comping_sidecar_path(path), Self::midi_sidecar_path(path)] {
            if !sidecar.is_file() {
                continue;
            }
    ...
    pub fn load_project_diagnostic_json(&self, path: &str) -> String {
        let result = if path.trim().is_empty() {
            ...
            Err(crate::bridge_error::BridgeError::new(
                "project_not_found", "project file does not exist",
            ))
            ...
            Err(crate::bridge_error::BridgeError::new(
                "project_load_failed", "project or sidecar hydration failed",
            )
            .retryable(false)
            .at_generation(self.project_generation()))
        };
    ...
    pub fn restore_project_backup(&self, path: &str, generation: u32) -> bool {
        ...
        let Some(candidate) = candidates.into_iter().find(|candidate| candidate.generation == generation)
        else {
            return false;
        };
        let current_path = std::path::Path::new(path);
        let rollback_path = std::path::PathBuf::from(format!("{path}.before-restore"));
        let rollback_ready = current_path.is_file()
            && std::fs::copy(current_path, &rollback_path).is_ok();
        ...
        let restored = self.engine.as_ref().is_some_and(|engine| {
            engine.load_project(candidate.path.to_string_lossy().as_ref())
        });
        if restored {
            return true;
        }
