...
impl MarkerOrchestrator {
    ...
    pub fn rename_marker(&mut self, id: u32, name: &str) -> bool {
        ...
        self.markers
            ...
            .map(|marker| {
                marker.name = name.trim().to_owned();
                true
            })
            .unwrap_or(false)
    }
    ...
    pub fn set_section_duration(&mut self, id: u32, duration_ticks: u64) -> bool {
        ...
        self.markers
            .iter_mut()
            .find(|marker| marker.id == id && matches!(marker.marker_type, MarkerTypeRust::Section))
            .map(|marker| {
                marker.duration_ticks = duration_ticks;
                true
            })
            .unwrap_or(false)
    }
    ...
    pub fn try_add_marker(
        ...
        pos: MusicalTimeRust,
        name: String,
        marker_type: MarkerTypeRust,
    ) -> bool {
        if self.markers.len() >= 1_000_000
            || pos.bar < 1
            || !(1..=4).contains(&pos.beat)
            || !(0..960).contains(&pos.ticks)
            || name.trim().is_empty()
            || name.len() > 256
            || name.contains('\0')
        {
    ...
    pub fn try_add_section(
        ...
        if !self.try_add_marker(pos, name, MarkerTypeRust::Section) {
            return false;
        }
    ...
    pub fn sync_to_tempo(&mut self, bpm: f64, sr: f64) {
        ...
        self.markers
            .sort_by_key(|marker| (marker.sample_pos, marker.id));
    }

    pub fn next_marker(&self, sample_pos: u64) -> Option<&MarkerRust> {
        self.markers
            .iter()
            .filter(|marker| marker.sample_pos > sample_pos)
            .min_by_key(|marker| (marker.sample_pos, marker.id))
    }
    ...
    pub fn previous_marker(&self, sample_pos: u64) -> Option<&MarkerRust> {
        self.markers
            .iter()
            .filter(|marker| marker.sample_pos < sample_pos)
            .max_by_key(|marker| (marker.sample_pos, marker.id))
    }
        ...
        self.markers.len() <= 1_000_000
            && self.markers.iter().all(|marker| {
                marker.id > 0
                    ...
                    && marker.name.len() <= 256
                    && !marker.name.contains('\0')
                    && marker.musical_pos.bar >= 1
                    && (1..=4).contains(&marker.musical_pos.beat)
                    && (0..960).contains(&marker.musical_pos.ticks)
                    && (matches!(marker.marker_type, MarkerTypeRust::Point)
                        || marker.duration_ticks > 0)
            })
            && self.markers.iter().enumerate().all(|(i, marker)| {
                self.markers[..i]
                    .iter()
                    .all(|previous| previous.id != marker.id)
            })
    }
}
...
mod tests {
    ...
    fn marker_navigation_and_collision_free_ids_are_deterministic() {
        ...
        assert_eq!(
            markers.next_marker(100).map(|marker| marker.name.as_str()),
            Some("Verse")
        );
        assert_eq!(
            markers
