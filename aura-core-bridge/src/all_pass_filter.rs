...
impl AllPassFilterEngine {
    pub fn new(delay_samples: usize, feedback: f32) -> Self {
        ...
        Self {
            ...
            feedback: if feedback.is_finite() { feedback.clamp(-1.0, 1.0) } else { 0.0 },
            idx: 0,
            mask: size - 1,
        }
    }
    ...
    pub fn audit_all_pass_filter(&self) -> bool {
        !self.delay_buffer.is_empty() && self.delay_buffer.len().is_power_of_two() && self.mask == self.delay_buffer.len() - 1 && self.idx < self.delay_buffer.len() && self.feedback.is_finite() && self.feedback.abs() <= 1.0 && self.delay_buffer.iter().all(|sample| sample.is_finite())
    }
}
