...
class AdvancedAudioDSPStandardProDeep {
public:
    // --- 1. PSYCHOACOUSTIC MASKING (Pro Bark Scale) ---
    static float calculateMaskingProDeep(float freq, float spl) {
    ...
    // --- 2. LINEAR PHASE FIR DESIGN (Pro Deep) ---
    void designFIRProDeep(std::vector<double>& coeffs, double cutoff, double sr, int taps) {
        coeffs.clear();
        ...
        for (int n = 0; n < taps; ++n) {
            ...
            const double sinc = std::abs(x) < 1.0e-12 ? 2.0 * fc :
                std::sin(2.0 * pi * fc * x) / (pi * x);
            const double phase = 2.0 * pi * static_cast<double>(n) / (taps - 1);
    ...
    // --- 3. DYNAMIC RANGE CONTROL PRO DEEP V2 ---
    static float applyKneeCompressionDeepV2(float level, float thresh, float ratio, float knee) {
