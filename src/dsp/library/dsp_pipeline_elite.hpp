...
public:
    ...
    static BiquadCoeffs designPeakElite(double freq, double gainDb, double q, double sr) {
        if (!std::isfinite(freq) || !std::isfinite(gainDb) || !std::isfinite(q) || !std::isfinite(sr) || sr <= 0.0 || q <= 0.0) return {};
    ...
    // --- 3. SATURATION (Vacuum Tube Modeling Pro Elite) ---
    static float tubeDistortElite(float x, float drive) {
        float xd = x * drive;
